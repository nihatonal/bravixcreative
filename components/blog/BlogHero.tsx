  ...
  return (
    <section className="relative overflow-hidden" style={{ minHeight: "75vh" }}>
      ...
      <div className="editorial-container relative flex h-full items-end pb-16 pt-40">
        <div className="max-w-3xl min-w-0 animate-fade-up">
          ...
          <h1 className="break-words font-serif text-4xl font-bold leading-tight text-primary-foreground md:text-5xl lg:text-6xl">
            {post.title}
          </h1>

          <p className="mt-5 break-words text-lg leading-relaxed text-primary-foreground/80 md:text-xl">
            {post.excerpt}
          </p>

          <div className="mt-8 flex min-w-0 flex-col gap-5 sm:flex-row sm:flex-wrap sm:items-center sm:gap-6">
            ...
            <div className="flex items-center gap-1.5 text-sm text-primary-foreground/70">
              ...
              <span>
                {post.readingTime} {t("read")}
              </span>
            </div>

            <Link
              href={`/${post.lang ?? "en"}/blog/${post.slug}`}
              className="group inline-flex w-fit max-w-full items-center gap-2 rounded-full bg-primary-foreground/10 px-6 py-3 text-sm font-medium text-primary-foreground backdrop-blur-sm transition-all hover:bg-primary-foreground/20 sm:ml-auto"
            >
              {t("readArticle")}
