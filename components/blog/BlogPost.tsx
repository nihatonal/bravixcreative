...
function extractHeadings(body: Post["body"]): TocItem[] {
  if (!Array.isArray(body)) return [];

  return body
    .map((block) => block as PortableHeadingBlock)
    .filter(
      (block): block is PortableHeadingBlock =>
        block._type === "block" &&
        !!block._key &&
...
export default function BlogPost({ post, popularBlogs, locale }: Props) {
            ...
            {popularBlogs.length > 0 ? (
              <section className="mt-20">
                ...
                <div className="grid gap-8 md:grid-cols-2">
                  {popularBlogs.filter((blog) => blog._id !== post._id).slice(1).map((relatedPost) => (
                    <BlogCard
                      key={relatedPost._id}
                      post={relatedPost}
                      variant="default"
