...
import { getProjectsPath, isProjectSection } from "@/lib/project-routes";
...
const HeaderMenu = () => {
  ...
  useEffect(() => {
    ...
    } else if (segments[1] && isProjectSection(locale, segments[1])) {
    ...
    <div className="hidden md:inline-flex w-1/3 items-center justify-center gap-3 lg:gap-7 text-sm capitalize font-semibold text-gray-800/80">
      {navItems.map((item) => {
        ...
        const isActive = isBlog
          ...
          : isPortfolio
            ? Boolean(segments[1] && isProjectSection(locale, segments[1]))
        ...
        if (isBlog) {
          return (
            <Link
              ...
              className="whitespace-nowrap hoverEffect relative group"
            >
              {item.label}
              <span
                className={`absolute -bottom-0.5 left-1/2 w-0 h-0.5 bg-bvs-purple group-hover:w-1/2 hoverEffect group-hover:left-0 ${
                  isActive ? "w-1/2" : ""
                }`}
              />
              <span
                className={`absolute -bottom-0.5 right-1/2 w-0 h-0.5 bg-bvs-purple group-hover:w-1/2 hoverEffect group-hover:right-0 ${
                  isActive ? "w-1/2" : ""
                }`}
              />
            </Link>
